...
/**
 * Instructions Quiz Component (user copy of the builtin)
 *
 * Same as @/builtins/instructionsQuiz/InstructionsQuiz.vue, with one addition:
 * a short answer lock. When a participant lands on a quiz question page, the
 * answer inputs are disabled for UNLOCK_DELAY_MS so they are nudged to read
 * all the options instead of blindly clicking through. The lock re-arms each
 * time a new question page is shown (including after a retry).
 */
...
// re-arm the lock every time the participant lands on a quiz question page
watch(
  () => api.pathString,
  (path) => {
    if (/^pages\/pg\d+$/.test(path)) {
      startLock()
    }
  },
  { immediate: true }
    ...
    <TitleTwoCol leftFirst leftWidth="w-1/3" :responsiveUI="api.config.responsiveUI">
      ...
      <template #right>
        ...
        <div class="border border-border text-left bg-muted p-6 rounded-lg">
          <!-- Read-the-options nudge: answers are locked briefly on each page -->
          <div v-if="locked" class="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
            <i-fa6-solid-hourglass-half class="animate-pulse" />
            <span>Take a moment to read all the options — you can answer in {{ remaining }}s.</span>
          </div>
          ...
          >
            ...
            <Select v-else v-model="api.stepData.questions[index].answer" :disabled="locked">
              <SelectTrigger class="w-full bg-background dark:bg-background text-base" :disabled="locked">
